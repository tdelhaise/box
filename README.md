...
- `boxd` publie automatiquement cet enregistrement dans la file `/uuid` via le coordonnateur Location Service (acteur Swift) afin que la résolution s’appuie sur les mêmes données que l’admin channel. Les helpers de résolution (par utilisateur ou nœud) sont prêts pour l’intégration CLI/handshake ; la file `/location` sera branchée dans une itération ultérieure.
