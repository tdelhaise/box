...
- Fichier unique: `~/.box/Box.plist` (surcharge via `--config`). Structure :
  ...
  - `server`: `port`, `log_level`, `log_target`, paramètres de transport (`transport`, `transport_status`, `transport_put`, `transport_get`), `admin_channel`, `port_mapping` (booléen), surcharge externe (`external_address`, `external_port`) et options Noise futures. La valeur par défaut `log_target` pointe vers `file:~/.box/logs/boxd.log`, `port_mapping` vaut `false` (activable via CLI ou PLIST) et les champs `external_*` sont laissés vides tant qu’aucune adresse publique n’est imposée.
