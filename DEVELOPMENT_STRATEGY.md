...
Next: la publication Location Service tourne désormais sur le même builder que l’admin channel (`addresses[]` avec sources `probe|config|manual`, bloc `connectivity` enrichi, enregistrements nœud/utilisateur synchronisés). Le port‑mapping opt‑in pilote désormais UPnP → PCP (MAP + PEER) → NAT‑PMP avec télémetrie complète (`portMappingStatus|Error|ErrorCode`, `portMappingPeer*`, `portMappingReachability*`, `manualExternal*`, `external_address`/`external_port` côté PLIST/CLI) et lance une sonde reachability HELLO lorsque l’endpoint externe est appris. Les tests d’intégration exécutent à présent la CLI (`BoxCLIIntegrationTests`) pour `box admin status|ping|locate|nat-probe` et `box --locate` avec un garde-fou de 30 s ; il restera à étendre cette couverture aux futures commandes (transferts de messages, mapping effectif) et à préparer la réintégration Noise/libsodium (S4) ainsi que la validation ACL côté Swift.
...
- Implement admin-channel actions for NAT probe, map_create/map_delete, probe_peer, and LS publish. **Status:** nat-probe JSON + LS publication en place; les scénarios `box admin status|ping|locate|nat-probe` (retour `disabled/skipped`) et `box --locate` passent par la CLI en test automatisé (timeout 30 s). Une validation “happy path” sera ajoutée dès qu’un routeur UPnP/PCP sera disponible.
