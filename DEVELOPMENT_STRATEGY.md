...
- Exit: `box --server` expose `status` sur le canal admin; non-root enforcement vérifié.
  ...
  Next: ajouter une vérification de connectivité (détection d’adresse IPv6 globale, avertissement si seule l’IPv4 NAT est disponible, publication des adresses observées dans `status`/`stats`), intégrer un module de port-mapping automatique (PCP/NAT-PMP/UPnP opt-in) pour l’IPv4, couvrir via tests d’intégration end-to-end CLI↔️serveur (PUT/GET persistants), préparer la réintégration Noise/libsodium (S4) et introduire la validation ACL côté Swift.
