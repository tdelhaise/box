...
bad_headers=$(find sources/include -maxdepth 1 -type f -name '*.h' ! -name 'BF*.h' -print || true)
if [ -n "${bad_headers}" ]; then
  echo "[ERROR] Non-BF headers in sources/include/:"
  echo "${bad_headers}"
  fail=1
fi

# 2) include/box should not contain .c files
bad_impls=$(find sources/include -maxdepth 1 -type f -name '*.c' -print || true)
if [ -n "${bad_impls}" ]; then
  echo "[ERROR] Implementation files (.c) found under sources/include:"
  echo "${bad_impls}"
  fail=1
fi

# 3) src/lib: only BF*.c sources
bad_sources=$(find sources/lib -maxdepth 1 -type f -name '*.c' ! -name 'BF*.c' -print || true)
if [ -n "${bad_sources}" ]; then
  echo "[ERROR] Non-BF sources in sources/lib/:"
  echo "${bad_sources}"
  fail=1
fi

# 4) includes to box/<header> should use BF* headers
headers=$(grep -R -n --include='*.c' --include='*.h' '#include \"box/' sources test | sed -E 's/.*#include "box\/([^\"]+)".*/\1/' || true)
