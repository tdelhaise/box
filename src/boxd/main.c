...
int main(int argc, char **argv) {
    ...
    int m = box_proto_pack(txbuf, sizeof(txbuf), BOX_MSG_HELLO, hello_payload, (uint16_t)strlen(hello_payload));
    if (m > 0)
	(void)box_dtls_send(dtls, txbuf, m);
    ...
    while (g_running) {
        ...
        switch (hdr.type) {
            case BOX_MSG_PING: {
                ...
                int k = box_proto_pack(txbuf, sizeof(txbuf), BOX_MSG_PONG, pong, (uint16_t)strlen(pong));
                if (k > 0)
			(void)box_dtls_send(dtls, txbuf, k);
                break;
            }
            case BOX_MSG_DATA: {
