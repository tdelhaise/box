...
int BFAeadEncrypt(const uint8_t *key, const uint8_t *nonce, const uint8_t *associatedData,
    ...
    int rc = crypto_aead_xchacha20poly1305_ietf_encrypt(
        ciphertext, &producedLength, plaintext, (unsigned long long)plaintextLength, associatedData,
        (unsigned long long)associatedDataLength, NULL /* nsec */, nonce, key);
    if (rc != 0)
        return BF_ERR;
...
int BFAeadDecrypt(const uint8_t *key, const uint8_t *nonce, const uint8_t *associatedData,
    ...
    int rc = crypto_aead_xchacha20poly1305_ietf_decrypt(
        plaintext, &producedLength, NULL /* nsec */, ciphertext,
        (unsigned long long)ciphertextLength, associatedData,
        (unsigned long long)associatedDataLength, nonce, key);
    if (rc != 0)
        return BF_ERR;
